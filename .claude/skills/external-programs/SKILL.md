...
## The FFI boundary: ffi.nix → NixBins.qml → bin.*

`ffi.nix` generates `NixBins.qml` at build time. It exposes two sets of properties:

- **Universal bins** — declared directly in `ffi.nix` (e.g. `bash`, `hyprctl`, `jq`). Available in every app.
- **App-specific bins** — passed via `extraBins` to `mkAppConfig` / `mkBarConfig` in `flake.nix` and `hm-module.nix`.
...
```nix
# For bar plugins (flake.nix and hm-module.nix, mkBarConfig):
extraBins = {
...
- Always add to **both** `flake.nix` and `hm-module.nix` — they are kept in sync.
...
Check `ffi.nix` for the current authoritative list.
...
- [ ] Added to `extraBins` in `hm-module.nix` (same location)
