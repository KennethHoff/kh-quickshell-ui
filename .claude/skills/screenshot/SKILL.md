...
```bash
tmux split-window -h
tmux send-keys 'tmux set-option -p allow-passthrough on && for f in <paths...>; do echo "$(basename $f .png)"; tmp=$(mktemp /tmp/icat-XXXXXX.png); nix run nixpkgs#imagemagick -- "$f" -resize x320 "$tmp" 2>/dev/null; kitty +kitten icat "$tmp"; rm "$tmp"; done' Enter
```
...
- Each image is resized to 400px tall via ImageMagick so multiple shots fit on screen without scrolling
