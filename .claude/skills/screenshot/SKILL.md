...
# per-revision body mirrors Pipeline above (probe + IPC + settle per Timing table)
for rev in "${revs[@]}"; do
...
Open the gallery per the [Display](#display-only-when-the-user-asks)
section, sourcing each pane's label/desc from
