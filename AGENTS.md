...
**Never commit manually.** Always invoke the `commit` skill (via the Skill tool) when you have changes to commit. The skill enforces:

- [Conventional Commits](https://www.conventionalcommits.org/) format: `type(scope): subject`
- Logical grouping (UI changes separate from Nix packaging, etc.)
- Keeping `ROADMAP.md` in sync with the implementation in the same commit
- Running `validate-changes` before committing any `.nix` or `.qml` files
