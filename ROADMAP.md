...
- [8] ✅ Gallery history — each invocation appends a session row (`<epoch>\t<compact JSON items>`) to `$XDG_DATA_HOME/kh-view/meta/history`, sharing the MetaStore path/format used by cliphist pins and launcher frecency; QML reads via `MetaStore { appName: "kh-view"; storeKey: "history" }`. `--recall [N]` (default 1) reopens the Nth-from-newest session, `--recall-ts <epoch>` reopens the session with that exact timestamp (stable launcher callback), and `--list-history` prints a human-readable summary; none of these append to history so recall stays idempotent. Browsing lives in the launcher as the `kh-view-history` plugin (chip label "Galleries") — each row shows date + item count, Enter calls `kh-view --recall-ts <ts>`; leveraging the existing launcher gives fuzzy search, plugin switching, and frecency hooks for free. IPC on a running kh-view also exposes `listHistory()` / `recall(n)` / `historyCount`. Missing files at recall time render an "image missing" / "file missing" placeholder pane with the path, so a partially-available session still opens cleanly
