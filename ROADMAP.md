...
- [8] ✅ Gallery history — each invocation appends a session row (`<epoch>\t<compact JSON items>`) to `$XDG_DATA_HOME/kh-view/meta/history`, sharing the MetaStore path/format used by cliphist pins and launcher frecency; QML reads via `MetaStore { appName: "kh-view"; storeKey: "history" }`. `--browse-history` opens a modal picker (search bar + date/count list, j/k/Enter, substring filter); `--recall [N]` is an alias that opens the picker and auto-commits the Nth-from-newest entry. Neither rewrites history (recall is idempotent). Sessions opened from the picker carry an "Esc · Back to History" pill in the top-left; Esc returns to the picker instead of quitting (a second Esc from the picker quits). `--list-history` prints a human-readable summary; IPC exposes `listHistory()` / `recall(n)` / `historyCount` / `mode` for running instances. Missing files at recall time render an "image missing" / "file missing" placeholder pane with the path, so a partially-available session still opens cleanly
