...
- [5] ✅ Hierarchical IPC prefix — `ipcPrefix` propagates through `BarPlugin` → `BarRow` → `BarDropdown.col` via parent chain walk; `BarPlugin`, `BarGroup`, `BarDropdown`, and `BarTooltip` each append their own `ipcName` segment via the shared `parentPrefix + "." + ipcName` pattern so nested targets like `bar.controlcenter.tailscale` or `bar.sonarr.error` fall out automatically; root prefix is `ipcName` from `mkBarConfig` (default `"bar"`), exposed as `programs.kh-ui.bar.ipcName` in the hm-module; `EthernetPanel` and `TailscalePanel` converted from `ControlTile` to `BarPlugin` base so they join the prefix chain regardless of popup nesting depth
...
- [9] ⬜ Per-delegate preview popup via `BarTooltip` *(depends on Building Blocks [9])* — today the Workspaces plugin uses a single shared `PopupWindow` whose content swaps based on `state.preview`/`state.btnX`, which enforces "only one preview visible at a time" as an implicit UX rule. Migrate to per-delegate popups by wrapping each delegate's content in `BarTooltip { ScreencopyView {...} }` so each workspace button owns its own independent popup; removes the shared `state.pending`/`state.preview`/`state.btnX` bookkeeping, lets multiple previews coexist (e.g. keyboard-pinned via the existing `showPreview(name)` IPC for side-by-side comparison), and deduplicates the 300 ms hover-delay Timer that currently lives in `Workspaces.qml` against the same mechanic inside `BarTooltip`. ScreencopyView cost stays bounded — each per-delegate popup only captures while visible
...
- [7] ✅ Configuration validation — `functionality.validateConfig()` runs on `Component.onCompleted` and on any of `host`/`port`/`pollInterval`/`apiKeyEnv` changing; checks `host` non-empty, `port` in `1..65535`, `pollInterval` ≥ 5 s, `apiKeyEnv` non-empty, and the named env var resolves to a non-empty value via `Quickshell.env()`. Writes a specific message to `_state.configError` (e.g. `"SONARR_API_KEY env var is empty or unset"`, `"port 70000 out of range (1..65535)"`); the timer and `poll()` both gate on `configError === ""` so misconfigured instances never hit the API. `configError` is tracked separately from the runtime `error` so a config fix doesn't clobber a legitimate runtime error; on invalid→valid transition the validator kicks an immediate poll instead of waiting out a full interval. IPC exposes `getConfigError()`, and `getError()` returns `configError` first, falling back to `error`. Also fixes a pre-existing bug where `StandardPaths.getenv()` was used in place of the correct `Quickshell.env()` API
