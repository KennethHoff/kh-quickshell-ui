...
- ✅ Source app attribution — records active Hyprland window (`hyprctl activewindow -j`) at copy time via a persistent `wl-paste --watch` listener; stored in `$XDG_DATA_HOME/kh-cliphist/meta/attribution` (id‹TAB›app_class per line); shown right-aligned on each row alongside the timestamp; stale IDs pruned on each load
